...
describe('structure — tables, columns, PKs', () => {
  it('exactly the IMP-010 tenant-core tables + IMP-013 audit_log + IMP-020 client hierarchy + IMP-021 engagements + IMP-030 compliance rules + IMP-031 profiles/instances + IMP-040 task family + IMP-041 review_items exist in public', () => {
    ...
    // task_comments); IMP-041 PASS A added review_items (SCH-17). No other
    // tables may appear.
    const tables = psql(
    ...
    expect(tables).toBe(
      'audit_log,client_compliance_profiles,client_relationships,clients,compliance_instances,compliance_rule_versions,compliance_types,contacts,engagements,firm_memberships,firms,legal_entities,profiles,registrations,review_items,task_checklist_items,task_comments,task_dependencies,tasks',
    );
  });
