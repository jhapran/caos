...
describe('structure — tables, columns, PKs', () => {
  it('exactly the IMP-010 tenant-core tables + IMP-013 audit_log + IMP-020 client hierarchy + IMP-021 engagements + IMP-030 compliance rules + IMP-031 profiles/instances + IMP-040 task family + IMP-041 review_items + IMP-042 alerts family exist in public', () => {
    ...
    // added alert_rules (SCH-19) + alerts (SCH-18). No other tables may
    // appear.
    const tables = psql(
    ...
    expect(tables).toBe(
      'alert_rules,alerts,audit_log,client_compliance_profiles,client_relationships,clients,compliance_instances,compliance_rule_versions,compliance_types,contacts,engagements,firm_memberships,firms,legal_entities,profiles,registrations,review_items,task_checklist_items,task_comments,task_dependencies,tasks',
    );
  });
