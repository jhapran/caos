...
describe('structure — PKs, uniques, indexes', () => {
  ...
  it('audit triggers exist on all five hierarchy tables (Layer A, AUD-CAT-01)', () => {
    const triggers = psql(
    ...
    expect(triggers).toBe(
      'client_relationships,clients,contacts,firm_memberships,firms,legal_entities,profiles,registrations',
    );
  });
});
